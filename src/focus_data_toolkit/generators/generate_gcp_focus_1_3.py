"""Deterministic generator for synthetic GCP data in the FOCUS 1.3 format.
...
providers — only the values differ. This emits Google Cloud data for both:
...
CLI
---
    python -m focus_data_toolkit.generators.generate_gcp_focus_1_3 \
        --rows 1000 --seed 1302 \
        --out focus_sample_costandusage_gcp_1000.csv
    python -m focus_data_toolkit.generators.generate_gcp_focus_1_3.py --dataset contract_commitment \
        --out focus_sample_contractcommitment_gcp.csv
"""
...
DEFAULT_COMMITMENT_OUT = Path("focus_sample_contractcommitment_gcp.csv")
...
def _split_allocation_row(rng: random.Random, i: int) -> dict[str, str]:
    """Split Cost Allocation row (FOCUS 1.3): a shared Compute Engine host's cost
    allocated to a GKE workload. ``ResourceId`` is the shared host; ``Allocated*``
    name the workload that received the split."""
    spec = _CE
    region_id, region_name, zones = rng.choice(_REGIONS)
    row, ctx = _base_row(rng)
    ...
    row["ChargeCategory"] = "Usage"
    row["ChargeFrequency"] = "Usage-Based"
    row["ChargeDescription"] = "Shared Compute Engine host cost allocated to workload"
    row["PricingCategory"] = "Standard"
    row["BilledCost"] = _s(contracted_cost)
    row["EffectiveCost"] = _s(contracted_cost)
    row["ListCost"] = _s(list_cost)
    ...
    row["AllocatedResourceId"] = (
        f"//container.googleapis.com/projects/{ctx['sub_id']}/clusters/gke-{_hexid(rng, 6)}"
        f"/workloads/{workload}"
    )
    row["AllocatedResourceName"] = f"workload-{workload}"
    row["AllocatedTags"] = json.dumps(
        {"workload": workload, "cost-center": rng.choice(_COST_CENTERS)},
        separators=(",", ":"),
    )
...
def _commitment_group(rng: random.Random, i0: int, remaining: int) -> list[dict[str, str]]:
    """A CUD Purchase row + linked committed-usage rows (shared CommitmentDiscountId).
    ...
    commit_name = "SpendBasedCUD-1yr" if spend_based else "ResourceBasedCUD-1yr-Compute"
...
def generate_rows(
    ...
    *,
    ...
    """Return ``rows`` synthetic GCP FOCUS 1.3 Cost and Usage records."""
    if rows < 1:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Generate synthetic GCP FOCUS 1.3 CSV data.")
    parser.add_argument(
        "--dataset",
    ...
    print(f"Wrote {args.dataset} ({len(columns)} GCP FOCUS 1.3 columns) -> {out}")
    return 0
